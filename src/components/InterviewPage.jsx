...
const InterviewPage = () => {
  ...
  const [dialogBoxConfig, setDialogBoxConfig] = useState({ title: "", message: "", buttons: [] });
  ...
  const handleDelete = (id) => {
    setDialogBoxConfig({
      ...
      buttons: [
        { text: "Cancel", onClick: () => setDialogBoxOpen(false), variant: "outlined" },
        {
          text: "Delete",
          onClick: async () => {
            try {
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
          <Typography variant="h5">My Interviews</Typography>
          <Button
            variant="contained"
            onClick={() => {
              setDialogOpen(true);
        ...
        <Grid container spacing={2}>
          {interviews.map((interview) => (
            <Grid item xs={12} sm={6} md={4} key={interview.interview_id}>
             <Card
  variant="outlined"
  sx={{
    borderRadius: 2,
...
>
  <Box>
    ...
    <Box sx={{ display: "flex", alignItems: "center", mb: 0.5 }}>
      ...
      <Typography variant="body2">{interview.contact_person || "—"}</Typography>
    </Box>
